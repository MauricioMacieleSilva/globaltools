...
export function PerfilCartolaEnrijecido() {
  ...
  const calcularPerfil = (linha: LinhaPerfilCartolaEnrijecido): CalculoItem | null => {
    ...
    if (espessura <= 0 || enrij1 <= 0 || enrij2 <= 0 || aba1 <= 0 || base <= 0 || aba2 <= 0 || enrij3 <= 0 || enrij4 <= 0 || comprimento <= 0 || largura <= 0 || quantidade <= 0) {
      return null;
    }

    // Tira = Enrij1 + Enrij2 + Aba1 + Base + Aba2 + Enrij3 + Enrij4 – (2 × Espessura × 6)
  ...
  const validarCampo = (id: string, campo: 'aba1' | 'aba2' | 'base' | 'enrij1' | 'enrij2' | 'enrij3' | 'enrij4', valor: string) => {
  ...
  const atualizarLinha = (id: string, campo: keyof LinhaPerfilCartolaEnrijecido, valor: string | boolean) => {
    const updatedLinhas = linhasCartolaEnrijecido.map(l => {
      if (l.id === id) {
        ...
        // Se não é assimétrico, espelhar Enrij1 -> Enrij3, Enrij2 -> Enrij4, Aba1 -> Aba2
        if (!novaLinha.assimetrico) {
          if (campo === 'enrij1') {
            novaLinha.enrij3 = valor as string;
          } else if (campo === 'enrij2') {
            novaLinha.enrij4 = valor as string;
          } else if (campo === 'aba1') {
          ...
          // Se mudou para simétrico, espelhar os valores existentes
            ...
            novaLinha.enrij3 = novaLinha.enrij1;
            novaLinha.enrij4 = novaLinha.enrij2;
  ...
  const adicionarLinha = () => {
    const novaLinha: LinhaPerfilCartolaEnrijecido = {
      id: gerarId(),
      espessura: '',
      enrij1: '',
      enrij2: '',
  ...
  const removerLinha = (id: string) => {
    if (linhasCartolaEnrijecido.length > 3) {
      if (confirm('Tem certeza que deseja remover esta linha?')) {
        atualizarLinhaCartolaEnrijecido(linhasCartolaEnrijecido.filter(linha => linha.id !== id));
        removerCalculo(id);
      }
    }
  };
  ...
  useEffect(() => {
    linhasCartolaEnrijecido.forEach(linha => {
  ...
  const totalPeso = linhasCartolaEnrijecido.reduce((sum, linha) => {
    const calculo = calcularPerfil(linha);
    return sum + (calculo?.pesoTotal || 0);
  }, 0);

  const totalPerda = linhasCartolaEnrijecido.reduce((sum, linha) => {
    const calculo = calcularPerfil(linha);
    return sum + (calculo?.pesoPerda || 0);
  }, 0);

  // Obter cálculos do tipo CARTOLA_ENRIJECIDO para visualização
  const calculosPerfilCartolaEnrijecido = Object.values(calculos).filter(calc => calc.tipo === 'CARTOLA_ENRIJECIDO');
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-19 gap-1 text-xs font-medium text-muted-foreground border-b pb-2 overflow-x-auto">
        ...
        <div className="text-center">Enrij4</div>
      ...
      <div className="space-y-4">
        ...
        return <div key={linha.id} className="grid grid-cols-19 gap-1 items-center p-2 bg-background rounded-lg border">
              ...
              <TooltipProvider>
                <Tooltip open={!!errosValidacao[`${linha.id}-enrij4`]} delayDuration={0}>
                  <TooltipTrigger asChild>
                    <div className="w-full">
                      <Input 
                        type="number" 
                        placeholder="0.0" 
                        value={linha.enrij4} 
                        onChange={e => atualizarLinha(linha.id, 'enrij4', e.target.value)}
                        onBlur={e => validarCampo(linha.id, 'enrij4', e.target.value)} 
                        className={`text-center text-xs ${!linha.assimetrico ? 'bg-muted text-muted-foreground' : ''} ${errosValidacao[`${linha.id}-enrij4`] ? 'border-destructive' : ''}`}
                        disabled={!linha.assimetrico}
                      />
                    </div>
                  </TooltipTrigger>
                  <TooltipContent side="top" className="bg-destructive text-destructive-foreground max-w-[250px]">
                    <p className="text-xs font-medium">{errosValidacao[`${linha.id}-enrij4`]}</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
              ...
              <div className="flex justify-center">
                  <Button variant="outline" size="sm" onClick={() => removerLinha(linha.id)} disabled={linhasCartolaEnrijecido.length <= 3} className="h-6 w-6 p-0">
