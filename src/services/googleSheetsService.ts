...
const GID = '1086211541'; // ID da aba BASE
    ...
    try {
      ...
      const comercialData: ComercialData[] = rows
        ...
        .map((row: string[], index: number): ComercialData => {
          ...
          if (index < 3) {
            console.log(`Linha ${index + 1} - UF (AE/30): "${row[30]}", Cidade (AF/31): "${row[31]}", Vendedor (AB/27): "${row[27]}"`);
          }
          
          // Debug da data bruta vinda da planilha (apenas primeiras 5 linhas)
          if (index < 5) {
            console.log(`[CSV DEBUG] Row ${index + 1} - Raw data_emissao: "${row[4]}", data_perdido: "${row[47] || 'N/A'}"`);
          }
