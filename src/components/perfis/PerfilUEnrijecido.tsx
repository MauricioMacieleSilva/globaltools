...
export function PerfilUEnrijecido() {
  ...
  const calcularPerfil = (linha: LinhaPerfilUEnrijecido): CalculoItem | null => {
    ...
    if (espessura <= 0 || enrij1 <= 0 || aba1 <= 0 || base <= 0 || aba2 <= 0 || enrij2 <= 0 || comprimento <= 0 || largura <= 0 || quantidade <= 0) return null;
    ...
    const updatedLinhas = linhasUEnrijecido.map(l => {
      if (l.id === id) {
        ...
        if (!novaLinha.assimetrico) {
          if (campo === 'enrij1') novaLinha.enrij2 = valor as string;
          else if (campo === 'aba1') novaLinha.aba2 = valor as string;
          else if (campo === 'assimetrico' && valor === false) { novaLinha.enrij2 = novaLinha.enrij1; novaLinha.aba2 = novaLinha.aba1; }
        }
        return novaLinha;
      }
      return l;
  ...
  useEffect(() => { linhasUEnrijecido.forEach(linha => { const calculo = calcularPerfil(linha); if (calculo) atualizarCalculo(linha.id, calculo); }); }, [linhasUEnrijecido]);

  const totalPeso = linhasUEnrijecido.reduce((s, l) => { const c = calcularPerfil(l); if (!c) return s; const p = parseFloat(l.percentualPerda) || 100; return s + (c.pesoTotal * p / 100); }, 0);
  const totalPerda = linhasUEnrijecido.reduce((s, l) => { const c = calcularPerfil(l); return s + (c?.pesoPerda || 0); }, 0);
  const percPerda = totalPeso > 0 ? (totalPerda / totalPeso * 100) : 0;

  const headers = ['U/Z', 'Sim', 'Esp.', 'Enrj1', 'Aba1', 'Base', 'Aba2', 'Enrj2', 'Comp.', 'Larg.', 'Qt.', '%P', 'Tira', 'P.T', 'P.P', 'Tipo', 'Est', 'Ver', 'Ação'];

  const isMobile = useIsMobile();
  if (isMobile) {
    return (
      <PerfilMobileGeneric
        titulo="U/Z Enrijecido"
        comOrientacaoUZ
        comSimetrico
        campos={[
          ...
          { key: 'enrij2', label: 'Enrij2', mirrorOf: 'enrij1' },
        ]}
        linhas={linhasUEnrijecido}
        setLinhas={atualizarLinhaUEnrijecido}
        novaLinhaFactory={() => ({ id: gerarId(), orientacaoUZ: 'U', espessura: '', enrij1: '', aba1: '', base: '', aba2: '', enrij2: '', comprimento: '6000', largura: '1200', quantidade: '', percentualPerda: '103', assimetrico: false })}
        resetLinha={(l) => ({ ...l, espessura: '', enrij1: '', aba1: '', base: '', aba2: '', enrij2: '', comprimento: '6000', largura: '1200', quantidade: '', percentualPerda: '103', assimetrico: false, orientacaoUZ: 'U' as const })}
        ...
        {linhasUEnrijecido.map(linha => {
          ...
          return (
            <div key={linha.id} className="grid gap-1 items-center p-1.5 bg-background rounded border" style={{ gridTemplateColumns: 'repeat(19, minmax(0, 1fr))' }}>
              ...
              <Input type="number" placeholder="0" value={linha.enrij2} onChange={e => atualizarLinha(linha.id, 'enrij2', e.target.value)} className={`text-center text-[10px] h-7 px-1 ${!linha.assimetrico ? 'bg-muted text-muted-foreground' : ''}`} disabled={!linha.assimetrico} />
