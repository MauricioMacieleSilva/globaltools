...
export function PerfilCartola() {
  ...
  const calcularPerfil = (linha: LinhaPerfilCartola): CalculoItem | null => {
    ...
    if (espessura <= 0 || enrij1 <= 0 || aba1 <= 0 || base <= 0 || aba2 <= 0 || enrij3 <= 0 || comprimento <= 0 || largura <= 0 || quantidade <= 0) {
      return null;
    }

    // Tira = Enrij1 + Aba1 + Base + Aba2 + Enrij3 – (2 × Espessura × 4)
  ...
  const atualizarLinha = (id: string, campo: keyof LinhaPerfilCartola, valor: string | boolean) => {
    const updatedLinhas = linhasCartola.map(l => {
      if (l.id === id) {
        ...
        // Se não é assimétrico, espelhar Enrij1 -> Enrij3, Aba1 -> Aba2
  ...
  const adicionarLinha = () => {
    const novaLinha: LinhaPerfilCartola = {
      id: gerarId(),
      espessura: '',
      enrij1: '',
      aba1: '',
  ...
  const totalPeso = linhasCartola.reduce((sum, linha) => {
    const calculo = calcularPerfil(linha);
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        return <div key={linha.id} className="grid grid-cols-20 gap-1 items-center p-2 bg-background rounded-lg border">
              ...
              <div className="flex justify-center">
                {calculo && (
                  <VisualizacaoPerfilPopover calculo={calculo} tipoPerfil="Cartola">
