  ...
  return `
    <div style="padding: 40px 20px; text-align: center; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 12px; margin: 20px 0;">
      ...
      <p style="margin: 0 0 24px 0; font-size: 16px; opacity: 0.9;">
        Não há transações comerciais registradas para ontem.
      </p>
    ...
    <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
      <div style="max-width: 650px; margin: 0 auto; background-color: white; box-shadow: 0 0 20px rgba(0,0,0,0.1);">
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 40px 30px; text-align: center;">
          ...
          <h1 style="margin: 0; color: white; font-size: 32px; font-weight: 700;">Relatório Diário</h1>
          <p style="margin: 12px 0 0 0; color: rgba(255,255,255,0.95); font-size: 16px;">${reportDate}</p>
          <p style="margin: 4px 0 0 0; color: rgba(255,255,255,0.8); font-size: 13px;">⏰ Relatório automático • Período: ontem (D-1)</p>
        </div>

        <div style="padding: 40px 30px;">
          ${!hasData ? generateEmptyStateHTML() : `
          <div style="margin-bottom: 40px;">
            <h2 style="margin: 0 0 24px 0; font-size: 22px; font-weight: 600; color: #1a1a1a; border-bottom: 3px solid #667eea; padding-bottom: 12px;">💼 Resumo Executivo</h2>
            
            ${kpisPrevious && variacaoFaturamento !== 0 ? `
            <div style="background: #f8f9fa; padding: 16px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid ${variacaoFaturamento > 0 ? '#28a745' : '#dc3545'};">
              <strong style="color: ${variacaoFaturamento > 0 ? '#28a745' : '#dc3545'};">
                ${variacaoFaturamento > 0 ? '📈 Crescimento' : '📉 Redução'} de ${Math.abs(variacaoFaturamento).toFixed(1)}% no faturamento
              </strong> comparado ao dia anterior
            </div>
            ` : ''}
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
              <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 24px; border-radius: 12px; color: white; box-shadow: 0 4px 6px rgba(102, 126, 234, 0.3); position: relative;">
                ${variacaoFaturamento !== 0 && kpisPrevious ? `
                <div style="position: absolute; top: 8px; right: 8px; background: ${variacaoFaturamento > 0 ? '#c6f6d5' : '#fed7d7'}; color: ${variacaoFaturamento > 0 ? '#22543d' : '#c53030'}; padding: 4px 8px; border-radius: 12px; font-size: 11px; font-weight: 700;">
                  ${variacaoFaturamento > 0 ? '↑' : '↓'} ${Math.abs(variacaoFaturamento).toFixed(1)}%
                </div>
                ` : ''}
                ...
                <div style="font-size: 11px; opacity: 0.8;">Total de ontem</div>
              </div>
              
              <div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); padding: 24px; border-radius: 12px; color: white; box-shadow: 0 4px 6px rgba(240, 147, 251, 0.3); position: relative;">
                ${variacaoOrcamentos !== 0 && kpisPrevious ? `
                <div style="position: absolute; top: 8px; right: 8px; background: ${variacaoOrcamentos > 0 ? '#c6f6d5' : '#fed7d7'}; color: ${variacaoOrcamentos > 0 ? '#22543d' : '#c53030'}; padding: 4px 8px; border-radius: 12px; font-size: 11px; font-weight: 700;">
                  ${variacaoOrcamentos > 0 ? '↑' : '↓'} ${Math.abs(variacaoOrcamentos).toFixed(1)}%
                </div>
                ` : ''}
                ...
                <div style="font-size: 11px; opacity: 0.8;">Emitidos ontem</div>
              </div>
          ...
          <div style="margin-bottom: 40px;">
            <h2 style="margin: 0 0 20px 0; font-size: 20px; font-weight: 600; color: #1a1a1a; border-bottom: 3px solid #667eea; padding-bottom: 12px;">🏆 Top 5 Clientes de Ontem</h2>
          ...
          <div style="margin-bottom: 40px;">
            <h2 style="margin: 0 0 20px 0; font-size: 20px; font-weight: 600; color: #1a1a1a; border-bottom: 3px solid #667eea; padding-bottom: 12px;">🔥 Top 5 Produtos de Ontem</h2>
            <table style="width: 100%; border-collapse: collapse;">
              <thead>
                <tr style="background: #f8f9fa;">
                  <th style="padding: 12px; text-align: left; font-size: 13px; color: #6c757d; font-weight: 600;">#</th>
          ...
          <div style="margin-bottom: 40px;">
            <h2 style="margin: 0 0 20px 0; font-size: 20px; font-weight: 600; color: #1a1a1a; border-bottom: 3px solid #dc3545; padding-bottom: 12px;">❌ Análise de Perdidos</h2>
          ...
          <div style="margin-bottom: 40px;">
            <h2 style="margin: 0 0 20px 0; font-size: 20px; font-weight: 600; color: #1a1a1a; border-bottom: 3px solid #ffc107; padding-bottom: 12px;">🔄 Cancelamentos e Devoluções</h2>
        ...
        <div style="background: #f8f9fa; padding: 24px 30px; text-align: center; border-top: 1px solid #e9ecef;">
          <p style="margin: 0 0 8px 0; color: #6c757d; font-size: 13px;">Relatório gerado automaticamente pelo sistema Global Aço</p>
          <p style="margin: 0; color: #adb5bd; font-size: 12px;">© ${new Date().getFullYear()} Global Aço. Todos os direitos reservados.</p>
        </div>
      </div>
    </body>
    </html>
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const endOfTwoDaysAgo = new Date(twoDaysAgo.getFullYear(), twoDaysAgo.getMonth(), twoDaysAgo.getDate() + 1);

    const kpis = await fetchRealKPIs(startOfYesterday.toISOString(), endOfYesterday.toISOString());
    const kpisPrevious = await fetchRealKPIs(startOfTwoDaysAgo.toISOString(), endOfTwoDaysAgo.toISOString());
    ...
    for (const config of configs) {
      try {
        const htmlContent = generateReportHTML(kpis, kpisPrevious, config, reportDate);
        ...
        await supabase.from('email_reports_log').insert({
          ...
          status: 'success',
          report_date: startOfYesterday.toISOString().split('T')[0]
        });
