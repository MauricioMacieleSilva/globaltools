...
interface CancelamentosDialogProps {
  isOpen: boolean;
  onClose: () => void;
  data: ComercialData[];
}

export function CancelamentosDialog({ isOpen, onClose, data }: CancelamentosDialogProps) {
  const [expandedPedidos, setExpandedPedidos] = useState<Set<string>>(new Set());

  const { totalPedidos, totalClientes, groupedData } = useMemo(() => {
    const canceladosData = data.filter(item => item.situacao === 'Cancelado');
    const pedidosUnicos = new Set(canceladosData.map(item => item.numeropedido));
    const clientesUnicos = new Set(canceladosData.map(item => item.cliente));
    
    const grouped = canceladosData.reduce((acc, item) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-6xl max-h-[80vh] overflow-hidden">
        <DialogHeader>
          <DialogTitle className="text-red-600">Pedidos Cancelados</DialogTitle>
        ...
        <div className="overflow-auto">
          ...
          {Object.keys(groupedData).length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              Nenhum pedido cancelado encontrado para o período selecionado.
            </div>
          )}
        </div>
      </DialogContent>
