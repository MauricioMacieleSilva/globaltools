...
export function PerfilCartolaEnrijecido() {
  ...
  const adicionarLinha = () => {
    atualizarLinhaCartolaEnrijecido([...linhasCartolaEnrijecido, { id: gerarId(), espessura: '', enrij1: '', enrij2: '', aba1: '', base: '', aba2: '', enrij3: '', enrij4: '', comprimento: '6000', largura: '1200', quantidade: '', percentualPerda: '101', assimetrico: false }]);
  };

  const limparLinha = (id: string) => {
    atualizarLinhaCartolaEnrijecido(linhasCartolaEnrijecido.map(l => l.id === id ? { ...l, espessura: '', enrij1: '', enrij2: '', aba1: '', base: '', aba2: '', enrij3: '', enrij4: '', comprimento: '6000', largura: '1200', quantidade: '', percentualPerda: '101', assimetrico: false } : l));
    removerCalculo(id);
  ...
  const headers = ['Sim', 'Esp.', 'E1', 'E2', 'Ab1', 'Bas', 'Ab2', 'E3', 'E4', 'Cmp', 'Lrg', 'Qt', '%P', 'Tira', 'T.Prd', 'kg/Pç', 'kg/Prd', 'P.T', 'P.+', 'Desc%', 'R$/kg', 'Valor', 'Ver', 'Ação'];

  return (
    <div className="space-y-4">
      <div className="grid gap-0.5 text-[9px] font-medium text-muted-foreground border-b pb-2" style={{ gridTemplateColumns: 'repeat(24, minmax(0, 1fr))' }}>
      ...
      <div className="space-y-2">
        {linhasCartolaEnrijecido.map(linha => {
          ...
          return (
            <div key={linha.id} className="grid gap-0.5 items-center p-1 bg-background rounded border" style={{ gridTemplateColumns: 'repeat(24, minmax(0, 1fr))' }}>
              ...
              <Input type="number" placeholder="0" value={descontos[linha.id] || ''} onChange={e => setDescontos(prev => ({...prev, [linha.id]: e.target.value}))} className="text-center text-[9px] h-6 px-0.5 text-orange-600" min="0" max="100" />
              <div className="text-center text-[9px] font-medium text-green-600">{precoComDesconto ? precoComDesconto.toFixed(2) : '-'}</div>
              <div className="text-center text-[9px] font-medium text-green-600">{valorTotal ? (valorTotal/1000).toFixed(1) + 'k' : '-'}</div>
